...
open class DSLGeneratorTask : DefaultTask() {
    ...
    private fun Class<*>.createViewModel(quirks: InkrementalQuirks): Pair<ViewModel, List<String>>? {
        ...
        return ViewModel(
            name = name,
            qualifiedClassName = canonicalName,
            hasTypeParameters = kotlin.typeParameters.isNotEmpty(),
            plainType = kotlin.asTypeName(),
            parametrizedType = asParametrizedType(),
            starProjectedType = kotlin.starProjectedType.asTypeName(),
            attrs = attrs
    ...
    fun Class<*>.attrsSequence(): Sequence<AttrModel> =
        declaredMethods
            ...
            .mapNotNull { method ->
                ...
                val plainType = parameterType.kotlin.asTypeName()

                val parametrizedType = parameterType.asParametrizedType()

                val argType = if(isListener && samLike) {
                    functions.first().kotlinFunction!!.toFunctionModel().asLambdaTypeName()
                } else {
                    parametrizedType
                }

                val starProjectedType = parameterType.kotlin.starProjectedType.asTypeName()
    ...
    private fun createModel(): ModuleModel {
        ...
        val views = viewsSequence()
            ...
            .onEach { model ->
                superViews[model.first.qualifiedClassName] = model.first
                superRequests += model
            }
            .map { it.first }
            .toList()
    ...
    private fun dumpModel(model: ModuleModel) {
        model.views.forEach { view ->
            logger.warn("[gen] ${view.qualifiedClassName}")
            view.attrs.forEach {
                val t = it.type
                ...
                logger.warn("[gen]       arg     ${t.argType}")
                logger.warn("[gen]       prmtrzd ${t.parametrizedType}")
                logger.warn("[gen]       *-proj  ${t.starProjectedType}")*/
            }
            logger.warn("[gen]")
        }
    }
    ...
    private fun AttrModel.buildListener(): CodeBlock {
        val viewClass = owner.qualifiedClassName
        ...
        return buildCodeBlock {
            ...
            if (viewClass == VIEW_CNAME) {
                ...
                val v = if (!owner.hasTypeParameters) {
                    "v"
                } else {
                    "(v as ${owner.parametrizedType})"
                }
                // @formatter:off
...
private fun Class<*>.asParametrizedType(): TypeName = when {
    isArray && !componentType.isPrimitive -> kotlin.parameterizedBy(componentType.kotlin)
    typeParameters.isEmpty() -> kotlin.asTypeName()
